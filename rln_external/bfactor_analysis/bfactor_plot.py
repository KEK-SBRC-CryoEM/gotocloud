...
def bfactor_main(args, unknown):
    ...
    opts.outfilepath_list = {"rosenthal": os.path.join(opts.output, opts.prefix+"rosenthal-henderson-plot.pdf"),
                             ...
                             "data_csv":  os.path.join(opts.output, opts.prefix+"particles_resolution.csv"),
                             "data_yaml": os.path.join(opts.output, opts.prefix+"bfactor_data.yaml"),
    }
    ...
    if os.path.isfile(RUNNING_FILE):
        ...
        try:
            ...
            output_txt = bfactor_to_text(**bfactor_data)
            print("\n".join(output_txt))
